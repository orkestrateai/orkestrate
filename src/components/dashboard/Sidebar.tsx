...
export function Sidebar() {
  ...
  return (
    <>
      {/* Mobile Toggle */}
      <div className="md:hidden fixed top-0 left-0 right-0 h-14 bg-[#0A0A0B]/80 backdrop-blur-md border-b border-white/5 z-[60] flex items-center justify-between px-4">
      ...
      <aside
        className={`
          fixed md:relative top-0 bottom-0 left-0 z-[58]
          h-screen bg-[#0A0A0B] border-r border-white/5
      ...
      >
        ...
        {/* ── Sticky Bottom Area ── */}
        <div
          className={`p-3 mt-auto relative flex flex-col gap-1 ${collapsed ? "items-center" : ""}`}
        >
          ...
          <button
            ...
            className={`
              group relative flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-medium
              text-red-400 hover:text-red-300 hover:bg-red-500/10 transition-all min-h-10
              ${collapsed ? "justify-center w-10 px-0" : "w-full"}
            `}
          >
            <div className="w-6 h-6 rounded-lg flex items-center justify-center shrink-0 bg-red-500/10 border border-red-500/20 group-hover:bg-red-500/20 transition-all">
              <MessageSquare className="w-4 h-4 text-red-400 group-hover:text-red-300" />
            </div>
            {!collapsed && <span>Report issue</span>}
            {collapsed && <Tooltip label="Report issue" />}
          </button>
