...
import {
  getJoinedWorkspaceForAgent,
  joinWorkspaceForAgent,
  touchAgentSession,
...
import {
  normalizeIntentChain,
  resolveIntent,
} from "@/lib/intent-workflows/resolver";
import {
  createWorkflowRun,
  pruneExpiredRuns,
  setRunPhase,
  touchRun,
  workflowRunKey,
} from "@/lib/intent-workflows/runtime";
import type {
  IntentId,
  WorkflowPhase,
  WorkflowRunContext,
} from "@/lib/intent-workflows/types";
...
type WorkspaceBucket = {
  agentMetadata: Map<
    string,
    { agentProfile: string; pastWorkSummary: string[] }
  >;
...
function getSessionWorkflowRun(
  ...
  return (
    bucket.workflowRuns.get(workflowRunKey(scopedAgentId, sessionId)) || null
  );
}
...
function setSessionWorkflowRun(
  ...
  bucket.workflowRuns.set(
    workflowRunKey(run.scopedAgentId, run.sessionId),
    run,
  );
}
...
function workflowContentEntries(input: {
  ...
  return [
    {
      type: "text" as const,
      text: workflowLine(input.phase, input.nextRequiredTool, input.why),
    },
    {
      type: "text" as const,
      text: JSON.stringify(workflowInstructionPayload(input), null, 2),
    },
  ];
}
...
function hasNonEmptyArchitectureFootprint(content: unknown): boolean {
  const obj =
    content && typeof content === "object" && !Array.isArray(content)
      ...
      : {};
  return (
    firstStringList(obj, ["architectureFootprint", "footprint"]).length > 0
  );
}
...
function json(
  ...
  const status =
    typeof payloadOrStatus === "number"
      ? payloadOrStatus
      : typeof statusOrPayload === "number"
        ? statusOrPayload
        : 200;
  const payload =
    typeof payloadOrStatus === "number" ? statusOrPayload : payloadOrStatus;
...
function sendOAuthChallenge(
  req: NextRequest,
  description = "Valid OAuth bearer token required.",
) {
...
function coerceRepo(value: unknown): StoredAgentState["repo"] {
  ...
  const canonicalRemote =
    typeof obj.canonicalRemote === "string" && obj.canonicalRemote.trim()
  ...
  const branch =
    typeof obj.branch === "string" && obj.branch.trim()
      ...
      : null;
  const headSha =
    typeof obj.headSha === "string" && obj.headSha.trim()
      ? obj.headSha.trim()
      : null;
  const dirty = typeof obj.dirty === "boolean" ? obj.dirty : null;
  const aheadBehind =
    typeof obj.aheadBehind === "string" && obj.aheadBehind.trim()
      ? obj.aheadBehind.trim()
      : null;
...
function coerceState(
  ...
  const currentObjective =
    firstNonEmptyString(obj, ["currentObjective", "objective"]) ||
    "Standing by for next task.";
  const architectureFootprint = firstStringList(obj, [
    "architectureFootprint",
    "footprint",
  ]);
  const implementationPlan = firstStringList(obj, [
    "implementationPlan",
    "plan",
  ]);
  const notesForTeam =
    firstNonEmptyString(obj, ["notesForTeam", "notes"]) || "";
  const pastWorkSummary = firstStringList(obj, [
    "pastWorkSummary",
    "completed",
  ]);
  const agentProfile =
    firstNonEmptyString(obj, ["agentProfile"]) ||
    `${canonicalAgentId} - coordination agent`;
...
function mapKnowledgeDocRow(
  row: typeof knowledgeDocs.$inferSelect,
): StoredKnowledgeDoc {
  return {
    id: row.id,
...
async function collectKnowledgeDescendants(
  workspaceId: string,
  rootId: string,
) {
  ...
  while (queue.length > 0) {
    ...
    const children = await db
      ...
      .where(
        and(
          eq(knowledgeDocs.workspaceId, workspaceId),
          eq(knowledgeDocs.parentId, parentId),
        ),
      );
...
async function validateKnowledgeParent(
  ...
  if (!parent.isFolder)
    return { ok: false as const, reason: "Parent must be a folder." };
  if (movingDocId && parent.id === movingDocId) {
    return { ok: false as const, reason: "A doc cannot be its own parent." };
  }
  if (movingDocId) {
    const descendants = new Set(
      await collectKnowledgeDescendants(workspaceId, movingDocId),
    );
    if (descendants.has(parent.id)) {
      return {
        ok: false as const,
        reason: "Move would create a parent cycle.",
      };
    }
  }
...
async function listWorkspaceCoordinationStates(
  workspaceId: string,
): Promise<StoredAgentState[]> {
  ...
  for (const session of sessions) {
    ...
    const content: AgentStateContent = {
      ...
      architectureFootprint:
        state && Array.isArray(state.footprint) ? state.footprint : [],
      implementationPlan: state && Array.isArray(state.plan) ? state.plan : [],
      notesForTeam: state?.notes || "",
      pastWorkSummary:
        metadata?.pastWorkSummary ||
        (state && Array.isArray(state.completed) ? state.completed : []),
    };
...
async function listActiveScopeClaims(
  workspaceId: string,
): Promise<ActiveScopeClaim[]> {
  const now = new Date();
...
function getUnreadMessagesAlert(
  bucket: WorkspaceBucket,
  agentId: string,
): string[] {
  ...
  for (const msg of bucket.messages) {
    if (
      (msg.toAgentId === agentId || msg.toAgentId === "@everyone") &&
      !receipts.has(msg.id)
    ) {
      unreadCount++;
    }
  ...
  if (unreadCount > 0) {
    return [
      `You have ${unreadCount} unread message(s). Call read_messages immediately to read them.`,
    ];
  }
  return [];
}

function buildJoinWorkspaceOrchestrationGuide(
  workspaceId: string,
  canonicalAgentId: string,
) {
  return [
...
async function buildMcpToolList(userId: string) {
  ...
  return [
    ...
    // },
    {
      ...
      description:
        "Classify the current user request into an intent and return the required coordination workflow. WHEN: call first for every new task-like prompt. WHY: it selects a safe execution pattern and unlocks write tools in the correct order.",
      inputSchema: {
        type: "object",
        properties: {
          userPrompt: {
          ...
          targetAgentId: {
            type: "string",
            description:
              "Optional target agent hint for delegate/assist intents.",
          },
          scopeHints: {
            type: "array",
            items: { type: "string" },
            description:
              "Optional candidate paths to include in returned recommendations.",
          },
          forceIntent: {
            type: "string",
            enum: [
    ...
    },
    {
      name: "join_workspace",
      description:
        "Join the workspace and verify repository identity. WHEN: call at session start or reconnect. WHY: coordination tools are invalid until a verified session exists.",
      inputSchema: {
        type: "object",
        properties: {
          toolName: {
            type: "string",
            description:
              "Optional human-readable tool/client name (for example: Windsurf, Warp, Lovable).",
          },
          workspaceId: {
            type: "string",
            description:
              "Optional workspace id. Defaults to current active workspace for the authenticated user.",
          },
          gitContext: {
            type: "object",
            ...
            properties: {
              ...
              collectedAt: {
                type: "string",
                description: "ISO timestamp when git context was captured.",
              },
            },
            required: [
              "remote",
    ...
    },
    {
      ...
      description:
        "Read authoritative team state and stateHash. WHEN: call after identify_intent and after any mismatch/conflict. WHY: provides fresh CAS context and active claims before write actions.",
      inputSchema: {
        type: "object",
        properties: {},
        additionalProperties: false,
    ...
    },
    {
      name: "claim_scope",
      description:
        "Reserve repo paths with strict overlap rejection. WHEN: call before editing files for editable intents. WHY: prevents concurrent edits on intersecting paths.",
      inputSchema: {
        type: "object",
        properties: {
          expectedStateHash: {
            type: "string",
            description:
              "Required hash returned by read_team_state for optimistic concurrency.",
          },
          paths: {
            type: "array",
            items: { type: "string" },
    ...
    },
    {
      name: "release_scope",
      description:
        "Release one active claim. WHEN: call on completion, handoff, or scope change. WHY: unblocks teammates and closes claim lifecycle cleanly.",
      inputSchema: {
        type: "object",
        properties: {
          claimId: {
    ...
    },
    {
      ...
      description:
        "Publish objective, footprint, plan, notes, and repo context. WHEN: call after claim and at progress checkpoints; non-edit intents must use empty footprint. WHY: keeps shared state consistent and auditable.",
      inputSchema: {
        type: "object",
        properties: {
          expectedStateHash: {
          ...
          content: {
            ...
            properties: {
              ...
              notesForTeam: {
                type: "string",
                description:
                  "Cross-agent coordination notes, warnings, decisions, handoff context.",
              },
              pastWorkSummary: {
                type: "array",
                items: { type: "string" },
              ...
              repo: {
                type: "object",
                description:
                  "Required repo context for coordination integrity.",
                properties: {
    ...
    },
    {
      name: "send_message",
      description:
        "Send a message to another agent or broadcast to all agents. Use this to coordinate, ask for help, or share information directly.",
      inputSchema: {
        type: "object",
        properties: {
          toAgentId: {
            type: "string",
            description:
              "The targeted agentId (e.g., 'frontend-1'), or '@everyone' to broadcast to the whole team.",
          },
          message: {
            type: "string",
    ...
    },
    {
      name: "read_messages",
      description:
        "Read all unread messages addressed to you or broadcasted to @everyone. Also marks them as read.",
      inputSchema: {
        type: "object",
        properties: {},
        additionalProperties: false,
    ...
    },
    {
      ...
      description:
        "Read workspace knowledge docs stored in Postgres. Use id for direct read, parentId for folder traversal (parentId:null = root), or query for search.",
      inputSchema: {
        type: "object",
        properties: {
          id: {
            type: "string",
            description: "Optional doc id for direct read.",
          },
          parentId: {
            type: ["string", "null"],
            description: "Optional parent filter. Use null to list root docs.",
          },
          query: {
            type: "string",
            description:
              "Optional case-insensitive search across title/description/content.",
          },
          includeContent: {
            type: "boolean",
            description: "Include full content in list responses.",
          },
        },
        additionalProperties: false,
      },
    },
    {
      ...
      description:
        "Mutate workspace knowledge docs in Postgres. Supports create/update/move/delete. Parent must be a folder; move cycles are rejected; folder delete cascades descendants.",
      inputSchema: {
        type: "object",
        properties: {
          action: {
          ...
          id: {
            type: "string",
            description: "Required for update/move/delete.",
          },
          title: { type: "string", description: "Doc title (create/update)." },
          description: {
            type: "string",
            description: "Optional doc summary (create/update).",
          },
          content: { type: "string", description: "Doc body (create/update)." },
          parentId: {
            type: ["string", "null"],
            description: "Folder parent id or null for root.",
          },
          isFolder: {
            type: "boolean",
            description: "Create as folder when true.",
          },
        },
        required: ["action"],
        additionalProperties: false,
...
export async function POST(req: NextRequest) {
  try {
    ...
    const clientRegistration = await getClientRegistration(
      oauthClient,
      clientId,
    );
    const clientName = clientRegistration?.client_name || null;

    const parentToolFromPayload = (rpc as Record<string, unknown>)
      ?.parentTool as string | null;
    const effectiveFamilyHint = parentToolFromPayload || clientName || clientId;
    ...
    const resolveAgentIdentity = (requestedAgentId: unknown) =>
      resolveAgentFingerprint({
        ...
        clientId,
        userId,
        familyHint: effectiveFamilyHint,
      });
    ...
    if (writeTools.has(aliasName) && !hasWriteScope) {
      return json(
        403,
        rpcError(id, -32001, "insufficient_scope: requires mcp:write"),
      );
    }
    ...
    if (aliasName === "Orkestrate_initialize") {
        ...
        return json(
          ...
          rpcResult(id, {
            content: [
              {
                type: "text",
                text: "ERROR: No active workspace found. Please create one in the dashboard first.",
              },
            ],
          }),
        );
      }
      const canonicalIdentity = resolveAgentIdentity(
      ...
      const bootstrapStatus =
        ...
        canonicalIdentity.family === "claude"
          ? "join-only"
          : "plugin-first";
    ...
    if (aliasName === "join_workspace") {
      ...
      const workspaceId =
        typeof argsObj.workspaceId === "string"
          ? argsObj.workspaceId
          : undefined;
      const rawToolName =
        typeof argsObj.toolName === "string" ? argsObj.toolName.trim() : "";
      ...
      const gitContext =
        argsObj.gitContext &&
        typeof argsObj.gitContext === "object" &&
        !Array.isArray(argsObj.gitContext)
          ? (argsObj.gitContext as Record<string, unknown>)
          : null;
      const gitRemote =
        gitContext && typeof gitContext.remote === "string"
          ? gitContext.remote.trim()
          : "";
      const repoRoot =
        gitContext && typeof gitContext.repoRoot === "string"
          ? gitContext.repoRoot.trim()
          : "";
      const gitBranch =
        gitContext && typeof gitContext.branch === "string"
          ? gitContext.branch.trim()
          : "";
      const gitHeadSha =
        gitContext && typeof gitContext.headSha === "string"
          ? gitContext.headSha.trim()
          : "";
      const gitDirty = gitContext ? gitContext.dirty : undefined;
      const collectedAt =
        gitContext && typeof gitContext.collectedAt === "string"
          ? gitContext.collectedAt.trim()
          : "";
        ...
        return json(
          ...
          rpcResult(id, {
            content: [
              {
                type: "text",
                text: "ERROR: join_workspace requires gitContext.",
              },
        ...
        return json(
          ...
          rpcResult(id, {
            content: [
              {
                type: "text",
                text: "ERROR: Invalid gitContext. remote, repoRoot, branch, and valid headSha are required.",
              },
              ...workflowContentEntries({
                ok: false,
                intentId: null,
        ...
        return json(
          ...
          rpcResult(id, {
            content: [
              {
                type: "text",
                text: "ERROR: Invalid gitContext. dirty must be boolean.",
              },
              ...workflowContentEntries({
                ok: false,
                intentId: null,
                ...
                recoverySteps: [
                  "Set gitContext.dirty to true/false and retry join_workspace.",
                ],
                errorCode: "WORKFLOW_STEP_VIOLATION",
                allowedToolsNow: ["join_workspace"],
              }),
        ...
        return json(
          ...
          rpcResult(id, {
            content: [
              {
                type: "text",
                text: "ERROR: Invalid gitContext. collectedAt must be an ISO timestamp.",
              },
              ...workflowContentEntries({
                ok: false,
                intentId: null,
                ...
                recoverySteps: [
                  "Provide an ISO timestamp string and retry join_workspace.",
                ],
                errorCode: "WORKFLOW_STEP_VIOLATION",
                allowedToolsNow: ["join_workspace"],
              }),
      ...
      if (!joined.ok) {
        const errorDetails =
          "details" in joined && joined.details
      ...
      const orchestrationGuide = buildJoinWorkspaceOrchestrationGuide(
        joined.workspaceId,
      ...
      return json(
        ...
        rpcResult(id, {
          content: [
            ...
            {
              type: "text",
              text: `Joined workspace ${joined.workspaceId} as ${canonicalIdentity.id}.`,
            },
            ...workflowContentEntries({
              ok: true,
              intentId: null,
    ...
    if (aliasName === "identify_intent") {
      const argsObj = args as Record<string, unknown>;
      const userPrompt =
        typeof argsObj.userPrompt === "string" ? argsObj.userPrompt.trim() : "";
      if (!userPrompt) {
        return json(
          400,
          rpcError(id, -32602, "Requires string argument 'userPrompt'."),
        );
      }

      const { canonicalIdentity, scopedAgentId, joined } =
        await resolveJoinedContext(argsObj);
      if (!joined) {
        return json(
          200,
          rpcResult(id, {
            content: [
              {
                type: "text",
                text: "ERROR: Agent not joined. Call join_workspace first.",
              },
              ...workflowContentEntries({
                ok: false,
                intentId: null,
      ...
      const targetAgentId =
        typeof argsObj.targetAgentId === "string"
          ? argsObj.targetAgentId.trim()
          : "";
      ...
      return json(
        ...
        rpcResult(id, {
          content: [
            ...
            {
              type: "text",
              text: `Identified intent '${resolution.intentId}' for ${canonicalIdentity.id}.`,
            },
            ...workflowContentEntries({
              ok: true,
              intentId: resolution.intentId,
              ...
              text: JSON.stringify(
                {
                  ...
                  scopeHints,
                  systemAlerts: getUnreadMessagesAlert(
                    getWorkspaceBucket(workspaceId),
                    canonicalIdentity.id,
                  ),
                },
                null,
    ...
    if (aliasName === "read_team_state") {
      ...
      const bucket = getWorkspaceBucket(workspaceId);
      const existingRun = getSessionWorkflowRun(
        bucket,
        scopedAgentId,
        joined.sessionId,
      );
      ...
      const summary =
        agents.length > 0
          ? agents
              .map(
                (state) =>
                  `- ${state.agentId} (${state.status}) objective: ${state.content.currentObjective} | footprint: ${state.content.architectureFootprint.length} paths`,
              )
              .join("\n")
    ...
    if (aliasName === "claim_scope") {
      ...
      if (paths.length === 0) {
        return json(
          400,
          rpcError(id, -32602, "Requires non-empty array argument 'paths'."),
        );
      }
        ...
        return json(
          ...
          rpcResult(id, {
            content: [
              {
                type: "text",
                text: "ERROR: Team state changed. Call read_team_state again and retry.",
              },
      ...
      const conflicts = activeClaims
        .filter(
          (claim) =>
            claim.scopedAgentId !== scopedAgentId &&
            findScopeConflicts(paths, claim.paths),
        )
        .map((claim) => ({
      ...
      const ttlParsed =
        typeof argsObj.ttlSeconds === "number"
          ? argsObj.ttlSeconds
          : Number(argsObj.ttlSeconds);
      ...
      return json(
        ...
        rpcResult(id, {
          content: [
            ...
            {
              type: "text",
              text: `Scope claimed for ${canonicalIdentity.id}.`,
            },
            ...workflowContentEntries({
              ok: true,
              intentId: existingRun?.intentId || null,
    ...
    if (aliasName === "release_scope") {
      const argsObj = args as Record<string, unknown>;
      const claimId =
        typeof argsObj.claimId === "string" ? argsObj.claimId.trim() : "";
      if (!claimId) {
        return json(
          400,
          rpcError(id, -32602, "Requires string argument 'claimId'."),
        );
      }
      ...
      const existing = await db.query.agentScopeClaims.findFirst({
        where: and(
          eq(agentScopeClaims.id, claimId),
          eq(agentScopeClaims.workspaceId, workspaceId),
        ),
      });
      if (!existing) {
      ...
      if (existing.agentId !== scopedAgentId) {
        return json(
          ...
          rpcError(
            id,
            -32003,
            "Cannot release a claim owned by another agent.",
          ),
        );
      }
      ...
      return json(
        ...
        rpcResult(id, {
          content: [
            ...
            {
              type: "text",
              text: `Released scope claim ${claimId} for ${canonicalIdentity.id}.`,
            },
            ...workflowContentEntries({
              ok: true,
    ...
    if (aliasName === "update_my_state") {
      const argsObj = args as Record<string, unknown>;
      if (
        typeof argsObj.content !== "object" ||
        argsObj.content === null ||
        Array.isArray(argsObj.content)
      ) {
        return json(
          400,
      ...
      const session = await db.query.agentSessions.findFirst({
        where: eq(agentSessions.id, joined.sessionId),
      });
      if (!session) {
      ...
      if (
        !next.repo.canonicalRemote ||
        !next.repo.branch ||
        !next.repo.headSha ||
        !isValidHeadSha(next.repo.headSha)
      ) {
        return json(
          400,
          rpcError(
            id,
            -32602,
            "content.repo.canonicalRemote, content.repo.branch, and valid content.repo.headSha are required.",
          ),
        );
      }
      ...
      if (next.footprint.length > 0) {
        const ownedActiveClaims = (
          await listActiveScopeClaims(workspaceId)
          ...
          return json(
            ...
            rpcResult(id, {
              content: [
                {
                  type: "text",
                  text: "ERROR: No active scope claim. Call claim_scope before publishing non-empty architectureFootprint.",
                },
                ...workflowContentEntries({
                  ok: false,
                  intentId: existingRun?.intentId || null,
        ...
        const missingPaths = next.footprint.filter(
          (path) =>
            !claimPaths.some((claimPath) => claimPathCovers(claimPath, path)),
        );
      ...
      let nextWorkflowPhase: WorkflowPhase =
        next.footprint.length > 0 ? "active" : "active";
      let nextIntentId: IntentId | null = existingRun?.intentId || null;
      if (
        next.footprint.length === 0 &&
        (next.status === "done" || next.status === "handoff")
      ) {
      ...
      if (existingRun) {
        if (nextWorkflowPhase === "done" && existingRun.chainQueue.length > 0) {
          ...
          setSessionWorkflowRun(
            bucket,
            setRunPhase(existingRun, nextWorkflowPhase),
          );
        }
      }
              ...
              recommendedArgs:
                nextRequiredTool === "read_team_state"
                  ...
                  : nextRequiredTool === "identify_intent"
                    ...
                    : {
                        expectedStateHash:
                          await workspaceStateHash(workspaceId),
                      },
            }),
            {
              type: "text",
              text: `stateHash=${await workspaceStateHash(workspaceId)}`,
            },
            {
              type: "text",
              text: JSON.stringify(
    ...
    if (aliasName === "send_message") {
      const argsObj = args as Record<string, unknown>;
      const toAgentId =
        typeof argsObj.toAgentId === "string" ? argsObj.toAgentId.trim() : "";
      const messageContent =
        typeof argsObj.message === "string" ? argsObj.message.trim() : "";
      if (!toAgentId || !messageContent) {
        return json(
          ...
          rpcError(
            id,
            -32602,
            "Requires string arguments 'toAgentId' and 'message'.",
          ),
        );
      }
      ...
      return json(
        ...
        rpcResult(id, {
          content: [
            {
              type: "text",
              text: `Message sent to ${toAgentId}. They will be notified via systemAlerts on their next state update or read.`,
            },
          ],
        }),
      );
    ...
    if (aliasName === "read_messages") {
      ...
      for (const msg of bucket.messages) {
        if (
          (msg.toAgentId === canonicalIdentity.id ||
            msg.toAgentId === "@everyone") &&
          !receipts.has(msg.id)
        ) {
          unread.push(msg);
          receipts.add(msg.id);
    ...
    if (aliasName === "read_knowledge_base") {
        ...
        return json(
          ...
          rpcResult(id, {
            content: [
              {
                type: "text",
                text: "ERROR: read_knowledge_base is temporarily disabled in this phase.",
              },
            ],
          }),
        );
      ...
      const parentIdArg =
        typeof argsObj.parentId === "string"
          ? argsObj.parentId.trim()
          : argsObj.parentId === null
            ? null
            : undefined;
      const queryArg =
        typeof argsObj.query === "string"
          ? argsObj.query.trim().toLowerCase()
          : "";
        ...
        return json(
          ...
          rpcResult(id, {
            content: [
              {
                type: "text",
                text: JSON.stringify({ workspaceId, doc }, null, 2),
              },
            ],
          }),
        );
      ...
      if (hasParentFilter && parentIdArg !== undefined) {
        if (parentIdArg === null)
          whereClauses.push(isNull(knowledgeDocs.parentId));
          ...
          content: [
            {
              type: "text",
              text: JSON.stringify(
                { workspaceId, count: docs.length, docs },
                null,
                2,
              ),
            },
    ...
    if (aliasName === "write_knowledge_base") {
      ...
      if (action === "create") {
        ...
        const parentId =
          typeof argsObj.parentId === "string" ? argsObj.parentId.trim() : null;
        const parentCheck = await validateKnowledgeParent(
          workspaceId,
          parentId,
        );
        if (!parentCheck.ok)
          return json(400, rpcError(id, -32602, parentCheck.reason));

        const title =
          typeof argsObj.title === "string" && argsObj.title.trim()
            ? argsObj.title.trim()
            : "Untitled";
      ...
      if (action === "update") {
        const idTrimmed =
          typeof argsObj.id === "string" ? argsObj.id.trim() : "";
        if (!idTrimmed) {
        ...
        await db
          ...
          .where(
            and(
              eq(knowledgeDocs.id, existing.id),
              eq(knowledgeDocs.workspaceId, workspaceId),
            ),
          );
        ...
        return json(
          ...
          rpcResult(id, {
            content: [
              {
                type: "text",
                text: JSON.stringify({ workspaceId, doc: updated }, null, 2),
              },
            ],
          }),
        );
      ...
      if (action === "delete") {
        ...
        const descendants = await collectKnowledgeDescendants(
          workspaceId,
          existing.id,
        );
          ...
          await db
            ...
            .where(
              and(
                eq(knowledgeDocs.workspaceId, workspaceId),
                or(
                  ...descendants.map((descId) => eq(knowledgeDocs.id, descId)),
                )!,
              ),
            );
        }
                ...
                text: JSON.stringify(
                  {
                    ...
                    doc: mapKnowledgeDocRow(existing),
                  },
                  null,
                  2,
                ),
