# Agentalk: Multi-Agent Collaboration via MCP

## The Core Vision
Agentalk aims to be the central nervous system ("State Sync") for local, isolated AI agents (e.g., Claude Code, Cursor, OpenCode, Codex). 

Instead of replacing the developer’s local agent, Agentalk connects them to a shared "State Store" using the Model Context Protocol (MCP). This enables true swarm collaboration where agents can:
...
We want to achieve "Zero-Prompt Collaboration" where simply installing the Agentalk MCP is enough to fundamentally alter the agent's behavior.
