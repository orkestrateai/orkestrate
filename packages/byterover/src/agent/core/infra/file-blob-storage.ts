...
/**
 * Simple file-based blob storage for testing and local development.
 * Stores each blob as a separate file with a companion .meta.json file.
 */
export class FileBlobStorage implements IBlobStorage {
  ...
  async store(key: string, content: Buffer | string, metadata?: Partial<BlobMetadata>): Promise<StoredBlob> {
    ...
    const now = Date.now()
    const data: Buffer = Buffer.isBuffer(content) ? content : Buffer.from(content, 'utf-8')
  ...
  async delete(key: string): Promise<void> {
    ...
    try {
      await unlink(join(this.baseDir, `${key}.blob`))
    } catch { /* ignore */ }

    try {
      await unlink(join(this.baseDir, `${key}.meta.json`))
    } catch { /* ignore */ }
  }
  ...
  async getMetadata(key: string): Promise<BlobMetadata | undefined> {
    ...
    try {
      const metaRaw = await readFile(join(this.baseDir, `${key}.meta.json`), 'utf-8')
      return JSON.parse(metaRaw) as BlobMetadata
    } catch {
      return undefined
    }
  }
